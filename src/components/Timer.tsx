  ...
  return (
    <div className="flex flex-col items-center gap-6 py-6">
      <div className="text-t-muted text-sm tracking-widest uppercase">
        {activeBaby ? `[ ${activeBaby.name} ]` : '[ add a baby in options ]'}
      </div>

      <BunnyAnimation isRunning={isRunning} />
      ...
      {showNotes && (
        <div className="w-full max-w-md border border-t-border rounded p-4 bg-t-card">
          <p className="text-t-gdim text-sm mb-2 tracking-wide">── Session complete! Add a note? ──</p>
          ...
          <div className="flex gap-3 mt-3">
            <button onClick={saveNote}
              className="flex-1 py-2 rounded text-sm font-mono border border-t-green text-t-green hover:bg-t-gdark">[ save note ]</button>
            <button onClick={skipNote}
              className="flex-1 py-2 rounded text-sm font-mono border border-t-border text-t-muted hover:border-t-green hover:text-t-green">[ skip ]</button>
          </div>
        </div>
      )}
      ...
      <div className="w-full max-w-md border border-t-border rounded p-4 bg-t-card font-mono text-sm">
        ...
        <div className="text-t-muted text-xs mb-1">[{bar}] {pct}%</div>
        <div className="flex justify-between text-xs mt-2">
          <span className="text-t-gdim">{Math.floor(totalToday / 60)}m {totalToday % 60}s today</span>
          <span className="text-t-muted">goal: {goalMinutes}m</span>
        </div>
      </div>
    </div>
  );
