...
import type { DailyStat, TummySession } from '../types';
...
function streak(stats: DailyStat[], goalSecs: number) {
  ...
  for (let i = 0; i < stats.length; i++) {
    ...
    if (format(parseISO(s.date), 'yyyy-MM-dd') !== format(expected, 'yyyy-MM-dd')) break;
...
export default function Stats() {
  const { activeBaby, goal, store, refreshKey } = useApp();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    store.listSessions(activeBaby.id, 500).then((all: TummySession[]) => {
      ...
      for (const row of all) {
        if (new Date(row.started_at) < since) continue;
      ...
      setStats(Object.values(map).sort((a, b) => b.date.localeCompare(a.date)));
      setLoading(false);
    });
  }, [activeBaby, store, refreshKey]);

  if (!activeBaby) return <div className="text-t-muted text-center py-12 text-sm font-mono">add a baby in options</div>;
  if (loading)     return <div className="text-t-muted text-center py-12 text-sm font-mono animate-pulse">loading...</div>;
  ...
  return (
    <div className="py-4 px-2 font-mono text-sm space-y-6">
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'TODAY',  val: todayStat ? `${Math.floor(todayStat.total_seconds/60)}m` : '0m',
            sub: (todayStat?.total_seconds ?? 0) >= goalSecs ? '✓ goal met' : `goal: ${goalMinutes}m`,
          ...
          <div key={c.label} className="border border-t-border rounded bg-t-card p-3 text-center">
            <div className="text-t-muted text-xs tracking-widest">{c.label}</div>
            <div className="text-2xl mt-1" style={{ color: c.color }}>{c.val}</div>
            <div className="text-t-muted text-xs mt-1">{c.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="border border-t-border rounded bg-t-card p-4">
          ...
          {stats.map(s => {
            ...
            return (
              <div key={s.date} className="flex items-center gap-2 text-xs">
                <span className="w-8 text-t-muted" style={{ color: isT ? '#00ff88' : undefined }}>{day}</span>
                <span className="flex-1 tracking-tighter"
                  style={{ color: met ? '#00ff88' : s.total_seconds > 0 ? '#ffaa00' : '#1a2e20' }}>{bar}</span>
        ...
        <div className="text-t-muted text-xs mt-3 border-t border-t-border pt-2">| = {goalMinutes}m goal mark</div>
      </div>
        ...
        <div className="space-y-1 text-xs">
          <div className="flex justify-between">
            ...
            <span className="text-t-gdim">
              {stats.length > 0 ? `${Math.floor(Math.max(...stats.map(s => s.total_seconds)) / 60)}m` : '0m'}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-t-muted">days met goal</span>
            <span className="text-t-gdim">{stats.filter(s => s.total_seconds >= goalSecs).length} / 7</span>
          </div>
          <div className="flex justify-between">
            <span className="text-t-muted">total sessions</span>
            <span className="text-t-gdim">{stats.reduce((a, s) => a + s.session_count, 0)}</span>
          </div>
          <div className="flex justify-between">
